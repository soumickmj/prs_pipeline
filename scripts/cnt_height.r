...
option_list = list(
  ...
  make_option(c('--pheno_col'), action='store', type='character', help='If phenotype is provided, but contains more than 1 phenotype, the phenotype that is to be used can be specified here.', default='standing_height'),
  ...
  make_option(c('-o', '--output'), action='store', type='character', help='output prefix [required]', default="/group/glastonbury/soumick/PRS/CnT/initial_height_A1FREQFilt_test_cohortS1701_Z49"),
  make_option(c('--ext_sumstats'), action='store', type='character', help='Path to external sumstats. In this case, GWAS will not be performed and the top SNPs from the sumstats will be considered'),
...
#TODO: clean it, hardcoding for now
phs_attr <- data.frame(fread("/project/ukbblatent/clinicaldata/Phisical_Info/phisicalinfo_82779_MD_04_03_2024_17_06_41.tsv"))[c('f.eid','f.50.0.0')]
colnames(phs_attr) <- c("f.eid", "standing_height")
result <- left_join(pheno, phs_attr, by = c("FID" = "f.eid"))
pheno <- na.omit(result)
