...
option_list = list(
  make_option(c('-i', '--model2load'), action='store', type='character', help='path to the trained model RDS file [required]', default='/group/glastonbury/soumick/PRS/filteredPLR/initial_exGWAS_MfiltMAF_S1701_Z49.seed1701.mod.rds'),
  ...
  make_option(c('-o', '--output'), action='store', type='character', help='output prefix [required]', default="/group/glastonbury/soumick/PRS/filteredPLR/initial_external_exGWAS_MfiltMAF_S1701_Z49"),
...
sink(file.path(paste0(opt$output,'.summary.mod.base.external.txt')))
summary(mod.base <- lm(baseline_formula, data=data.full))
sink()
...
#load the trained PLR model
mods <- readRDS(opt$model2load)
mod <- mods$mod
message('[',now,'][Message] trained model loaded')

if (is.null(opt$cov_PC)){
  
  pred.mod.external <- predict(mod, G,  ind.row = rows_along(G), base.row = pred.base)
  
} else {
  
  cov.PC.external <- covar_from_df(data.full[, cov_PC_cols])
  pred.mod.external <- predict(mod, G, ind.row = rows_along(G), covar.row = cov.PC.external, base.row = pred.base)
  
}

pred.combo.external <- pred.base + pred.mod.external

data.full$Pred_base <- pred.base
data.full$PRS_PLR <- pred.mod.external
data.full$PRS_PLRbase <- pred.combo.external
saveRDS(data.full, file=file.path(paste0(opt$output, ".pred.external.rds")))


writeLines(sprintf(
  "R^2 scores:::\nBaseline: %f\nPLR: %f\nCombo: %f",
  cor(pred.base, data.full[[opt$pheno_col]])^2, # Baseline
  cor(pred.mod.external, data.full[[opt$pheno_col]])^2, # PLR
  cor(pred.combo.external, data.full[[opt$pheno_col]])^2 # Combo
), file.path(paste0(opt$output,'.scores.external.txt')))
