...
option_list = list(
  make_option(c('--prs_path'), action='store', type='character', help='Root path where the PRS of all the phenotypes are storred', default = '/group/glastonbury/soumick/PRS/LDPred2/F20208v3_DiffAE/nonMRI'),
  ...
  make_option(c('--disease_csv'), action='store', type='character', help='csv file (or, output of plink) with IID column (coma-separated), and BinCAT_Disease containing 1 or 0 for the particular disease', default='/project/ukbblatent/clinicaldata/binary_disease_cohorts/F20208v3_nonMRI/caucasian/atrial-fibrillation_flutter.csv'),
  #make_option(c('--disease_csv'), action='store', type='character', help='csv file (or, output of plink) with IID column (coma-separated), and BinCAT_Disease containing 1 or 0 for the particular disease', default='/project/ukbblatent/clinicaldata/binary_disease_cohorts/F20208v3_DiffAE/replic_caucasian/atrial-fibrillation_flutter.csv'),
...
file_list <- list.files("/group/glastonbury/soumick/PRS/LDPred2/F20208v3_DiffAE/nonMRI", pattern = "run_ext_basic_FiltAF_.*\\.fullDS\\.auto\\.mod\\.LDPred2\\.rds")

data.full <- readRDS(gsub("auto.mod", "resNdata.basic", paste(opt$prs_path,file_list[1],sep="/")))

prs_data <- lapply(file_list, function(f) {
  prs_raw <- readRDS(paste(opt$prs_path,f,sep="/"))
  data.full$PRS_auto <- prs_raw$pred_auto
  prs <- data.full[,c("IID","PRS_auto")]
  colnames(prs) <- c("IID", paste("PRS", gsub("run_ext_basic_FiltAF_|.fullDS.auto.mod.LDPred2.rds", "", f), sep = "_"))
  return(prs)
}) %>% bind_rows()
...
data.full <- readRDS("/group/glastonbury/soumick/PRS/LDPred2/initial_nonMRI_basic_FiltAF_S1701_Z49.seed1701.fullDS.resNdata.basic.LDPred2.rds")
